...
class ModernAnalysisSystem {
    ...
    async startAnalysis() {
        ...
        // Gera session_id único se não existir
        if (!data.session_id) {
        ...
        try {
            ...
            console.log('🚀 Iniciando análise com dados:', data);
            ...
            if (result.success) {
                ...
                // Inicia monitoramento com delay
                setTimeout(() => {
    ...
    async loadSavedSessions() {
        try {
            ...
            if (result.success && result.sessions && Array.isArray(result.sessions)) {
                ...
                console.log(`✅ ${this.sessions.size} sessões carregadas`);
            ...
            // Não mostrar erro se for problema de conectividade inicial
            if (error.message && !error.message.includes('Failed to fetch')) {
                this.showNotification('Erro ao carregar sessões salvas', 'error');
            }
            this.renderEmptySessionsList();
        }
    }
    ...
    startProgressMonitoring() {
        if (!this.currentSessionId) {
            console.error('❌ Nenhum session_id para monitorar');
            return;
        }

        console.log('🔄 Iniciando monitoramento para sessão:', this.currentSessionId);
        ...
        this.progressInterval = setInterval(async () => {
            try {
                ...
                if (!response.ok) {
                    console.error(`❌ Erro HTTP no progresso: ${response.status}`);
                    return;
                }
                ...
                if (data.success && data.progress) {
                    ...
                    this.updateProgress(
                        ...
                        progress.total_steps || 13,
                        this.formatTime(progress.estimated_remaining || 0)
                    );
                    ...
                    console.error('❌ Erro no progresso:', data.error);
                    this.stopProgressMonitoring();
                    this.showNotification(`Erro no progresso: ${data.error}`, 'error');
                    this.showProgress(false);
                }

            } catch (error) {
                console.error('❌ Erro no monitoramento:', error);
                // Não para o monitoramento por um erro isolado
            }
