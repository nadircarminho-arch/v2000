...
import gzip
import traceback
from pprint import pformat # Importado para o uso no bloco de código original
from flask import g # Importado para o uso no bloco de código original

logger = logging.getLogger(__name__)

# Constante para compatibilidade
AUTO_SAVE_DIR = Path("relatorios_intermediarios")
...
def get_session_progress(session_id: str) -> Dict[str, Any]:
    ...
    try:
        logs_path = auto_save_manager.subdirs['logs']
        session_files = list(logs_path.glob(f"{session_id}_*.txt")) # Busca apenas arquivos .txt para progresso

        if not session_files:
            # Tenta encontrar outros tipos de arquivos se .txt não for encontrado
            session_files = list(logs_path.glob(f"{session_id}_*"))
            if not session_files:
        ...
        for file_path in session_files:
            try:
                ...
                timestamp_str = 'unknown'
                match = re.search(r'_(\d{8}_\d{6}_\d{3})', file_path.stem)
                if match:
                    timestamp_str = match.group(1)

                progress_data.append({
                    'timestamp': timestamp_str,
                    'content': content.splitlines() # Divide o conteúdo em linhas para melhor visualização
                })
            except Exception as e:
                logger.error(f"Erro ao ler arquivo {file_path} para progresso: {e}")
...
def listar_sessoes_salvas(session_id: Optional[str] = None, categoria: Optional[str] = None) -> List[str]:
    """Lista sessões salvas, opcionalmente filtradas por ID e categoria"""
    try:
        all_files_info = []

        if categoria and categoria in auto_save_manager.subdirs:
            search_path = auto_save_manager.subdirs[categoria]
            pattern = f"{session_id}*.txt" if session_id else "*.txt" # Prioriza .txt
            files = list(search_path.glob(pattern))
            # Se nenhum .txt encontrado, busca .json
            if not files and session_id:
                files = list(search_path.glob(f"{session_id}*.json"))
            elif not files and not session_id:
                files = list(search_path.glob("*.json"))

            for f in files:
                all_files_info.append(str(f))
        else:
            # Busca em todos os diretórios se categoria não especificada ou inválida
            for cat, subdir in auto_save_manager.subdirs.items():
                pattern_txt = f"{session_id}*.txt" if session_id else "*.txt"
                files_txt = list(subdir.glob(pattern_txt))
                files_json = []
                if not files_txt and session_id:
                    files_json = list(subdir.glob(f"{session_id}*.json"))
                elif not files_txt and not session_id:
                    files_json = list(subdir.glob("*.json"))

                all_files_info.extend([str(f) for f in files_txt])
                all_files_info.extend([str(f) for f in files_json])

        return sorted(list(set(all_files_info))) # Remove duplicatas e ordena
    except Exception as e:
        logger.error(f"Erro ao listar sessões: {e}")
        return []